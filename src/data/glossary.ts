...
export const glossaryTerms: GlossaryTerm[] = [
  ...
  /* ------------------------------------------------------------------ */
  {
    ...
    th: "รอบสัญญาณไฟ",
    explanation: {
      th: "เวลาทั้งหมดที่สัญญาณไฟใช้ครบหนึ่งรอบ นับตั้งแต่ไฟเขียวของทิศแรกจนวนกลับมาที่จุดเดิมอีกครั้ง ทางแยกทั่วไปมักใช้รอบสัญญาณไฟ (Cycle Time) ประมาณ 60–150 วินาที",
    ...
    example: {
      th: "ทางแยกแห่งหนึ่งใช้รอบสัญญาณไฟ 90 วินาที แบ่งเป็นไฟเขียวถนนสายหลัก 45 วินาที ไฟเขียวถนนสายรอง 30 วินาที ที่เหลือ 15 วินาทีเป็นไฟเหลืองและไฟแดงร่วม",
  ...
  },
  {
    ...
    th: "จังหวะสัญญาณไฟ",
    explanation: {
      th: "ช่วงเวลาที่กลุ่มทิศทางการเดินรถซึ่งไม่ขัดแย้งกันได้รับไฟเขียวพร้อมกัน หนึ่งรอบสัญญาณไฟประกอบด้วยหลายจังหวะ (Phase) เรียงต่อกันจนครบทุกทิศทาง",
    ...
    example: {
      th: "สี่แยกแบบง่ายมี 2 Phase คือ Phase ที่ 1 ให้ไฟเขียวทิศเหนือ–ใต้ และ Phase ที่ 2 ให้ไฟเขียวทิศตะวันออก–ตะวันตก สลับกันไปตลอดทั้งวัน",
  ...
  },
  {
    id: "split",
    en: "Split",
    th: "สัดส่วนเวลาของแต่ละจังหวะ",
    explanation: {
      th: "การแบ่งเวลาในหนึ่งรอบสัญญาณไฟให้แต่ละ Phase มักระบุเป็นวินาทีหรือเปอร์เซ็นต์ของรอบ ทิศทางที่มีรถมากควรได้ Split มากกว่าทิศทางที่มีรถน้อย",
    ...
    example: {
      th: "รอบสัญญาณไฟ 100 วินาที ตั้ง Split ให้ถนนสายหลัก 60% (60 วินาที) และถนนสายรอง 40% (40 วินาที) ตามปริมาณรถของแต่ละฝั่ง",
  ...
  },
  {
    ...
    th: "เวลาไฟเขียว",
    explanation: {
      th: "ระยะเวลาที่ทิศทางหนึ่งได้รับไฟเขียวในแต่ละรอบ เวลาไฟเขียว (Green Time) เป็นตัวกำหนดว่าจะปล่อยรถผ่านทางแยกได้กี่คันต่อรอบ",
  ...
  },
  {
    ...
    th: "เวลาไฟเหลือง",
    explanation: {
      th: "ช่วงเตือนระหว่างไฟเขียวกับไฟแดง เพื่อให้ผู้ขับขี่ตัดสินใจหยุดได้อย่างปลอดภัย หรือขับผ่านทางแยกไปได้ทันก่อนไฟแดง ปกติใช้ประมาณ 3–5 วินาที ขึ้นกับความเร็วของถนน",
  ...
  },
  {
    ...
    explanation: {
      th: "ช่วงสั้น ๆ ที่ทุกทิศทางเห็นไฟแดงพร้อมกัน เพื่อให้รถคันสุดท้ายที่ค้างอยู่กลางทางแยกออกไปได้หมดก่อนที่ทิศถัดไปจะได้ไฟเขียว ช่วยลดโอกาสเกิดอุบัติเหตุตัดหน้ากัน ปกติใช้ 1–3 วินาที",
    ...
    example: {
      th: "หลังไฟเหลือง 3 วินาที ทางแยกตั้ง All-Red อีก 2 วินาที รวมเป็นช่วงเปลี่ยนจังหวะ 5 วินาที ก่อนทิศถัดไปได้ไฟเขียว",
  ...
  },
  {
    id: "minimum-green",
    en: "Minimum Green",
    th: "เวลาไฟเขียวต่ำสุด",
    explanation: {
      th: "เวลาไฟเขียวที่สั้นที่สุดที่ระบบยอมให้ในแต่ละจังหวะ แม้จะมีรถน้อยก็ตาม เพื่อความปลอดภัยและให้รถที่จอดรอมีเวลาออกตัว ใช้มากในระบบ Vehicle Actuated (VA)",
    ...
    example: {
      th: "ตั้ง Minimum Green ไว้ 7 วินาที แม้ถนนสายรองมีรถรอเพียงคันเดียว ไฟเขียวจะเปิดอย่างน้อย 7 วินาทีก่อนระบบพิจารณาตัดจบ",
  ...
  },
  {
    id: "maximum-green",
    en: "Maximum Green",
    th: "เวลาไฟเขียวสูงสุด",
    explanation: {
      th: "เวลาไฟเขียวที่ยาวที่สุดที่จังหวะหนึ่งจะครองได้ แม้จะยังมีรถมาต่อเนื่อง เพื่อไม่ให้ทิศทางอื่นต้องรอนานเกินไป เมื่อครบ Maximum Green ระบบจะบังคับเปลี่ยนจังหวะ",
    ...
    example: {
      th: "ตั้ง Maximum Green ของถนนสายหลักไว้ 60 วินาที ถึงรถจะยังมาไม่ขาดสาย ไฟจะเปลี่ยนเมื่อครบ 60 วินาที เพื่อให้ถนนสายรองได้ไฟเขียวบ้าง",
  ...
  },
  {
    id: "vehicle-call",
    en: "Vehicle Call",
    th: "การเรียกขอสัญญาณไฟเขียว",
    explanation: {
      th: "เมื่อ Detector ตรวจพบรถในทิศทางที่กำลังเป็นไฟแดง ระบบจะบันทึกคำขอ (Call) เพื่อจัดไฟเขียวให้ทิศทางนั้นในจังหวะถัดไป ถ้าไม่มี Call ระบบสามารถข้ามจังหวะนั้นไปได้เลย",
    ...
    example: {
      th: "กลางดึกมีรถคันเดียวมาจอดรอที่ถนนสายรอง Detector ส่ง Vehicle Call ไปที่ตู้ควบคุม ระบบจึงตัดไฟเขียวสายหลักแล้วเปิดไฟเขียวให้สายรองภายในไม่กี่วินาที",
  ...
  /* ------------------------------------------------------------------ */
  {
    ...
    th: "ความยาวแถวคอย",
    explanation: {
      th: "จำนวนรถ (หรือระยะทางเป็นเมตร) ที่จอดต่อแถวรอไฟแดงในทิศทางหนึ่ง เป็นตัวชี้วัดพื้นฐานว่าทางแยกรับมือกับปริมาณรถได้ดีแค่ไหน ยิ่งแถวคอยยาว ผู้ใช้ทางยิ่งเสียเวลามาก",
    ...
    example: {
      th: "ชั่วโมงเร่งด่วนเช้า ถนนสายหลักมีความยาวแถวคอย (Queue Length) ประมาณ 20 คัน หรือราว 120 เมตร ถ้าไฟเขียวหนึ่งรอบระบายไม่หมด แถวจะสะสมยาวขึ้นเรื่อย ๆ",
  ...
  },
  {
    ...
    th: "ระดับความอิ่มตัวของการจราจร",
    explanation: {
      th: "อัตราส่วนระหว่างปริมาณรถที่มาถึงกับความสามารถในการระบายรถของทางแยก (v/c) ค่าใกล้ 1.0 แปลว่าใช้ความจุเกือบเต็ม ถ้าเกิน 1.0 รถมาเร็วกว่าที่ระบายได้ แถวคอยจะยาวขึ้นทุกรอบ",
    ...
    example: {
      th: "ทิศทางหนึ่งระบายรถได้สูงสุด 1,000 คัน/ชั่วโมง แต่มีรถมา 900 คัน/ชั่วโมง ระดับความอิ่มตัว (Degree of Saturation) = 0.90 ถือว่าใกล้เต็มความจุ ควรเฝ้าระวัง",
  ...
  /* ------------------------------------------------------------------ */
  {
    id: "coordination",
    en: "Coordination",
    th: "การประสานสัญญาณไฟ",
    explanation: {
      th: "การตั้งเวลาสัญญาณไฟของทางแยกที่อยู่ใกล้กันให้สัมพันธ์กัน เพื่อให้รถที่ออกจากทางแยกหนึ่งไปเจอไฟเขียวที่ทางแยกถัดไปพอดี เกิดเป็น \"คลื่นไฟเขียว\" (Green Wave) ลดการหยุดซ้ำ ๆ ตลอดเส้นทาง",
    ...
    example: {
      th: "ถนนสายหลักมี 4 ทางแยกติดกัน ทุกแยกใช้รอบสัญญาณไฟ 90 วินาทีเท่ากันและตั้งเวลาต่อเนื่องกัน รถที่วิ่งประมาณ 50 กม./ชม. จะผ่านได้โดยแทบไม่ต้องหยุดเลย",
  ...
  },
  {
    id: "offset",
    en: "Offset",
    th: "ค่าเวลาเหลื่อมระหว่างทางแยก",
    explanation: {
      th: "ระยะเวลาที่จุดเริ่มไฟเขียวของทางแยกหนึ่งเหลื่อมจากทางแยกก่อนหน้า ในระบบประสานสัญญาณไฟ ค่า Offset ที่ดีต้องเท่ากับเวลาที่รถใช้เดินทางระหว่างสองทางแยกนั้น",
    ...
    example: {
      th: "ทางแยก B อยู่ห่างจากทางแยก A 500 เมตร รถวิ่ง 50 กม./ชม. ใช้เวลาประมาณ 36 วินาที จึงตั้ง Offset ของทางแยก B ไว้ 36 วินาที เพื่อให้รถมาถึงตอนไฟเขียวเปิดพอดี",
  ...
  },
  {
    ...
    explanation: {
      th: "การบริหารสัญญาณไฟทุกทางแยกบนถนนสายหนึ่งให้ทำงานร่วมกันเป็นระบบเดียว มักสั่งการจากศูนย์ควบคุมกลาง เป้าหมายคือให้การจราจรไหลลื่นทั้งเส้นทาง ไม่ใช่แค่ทางแยกใดทางแยกหนึ่ง",
    ...
    example: {
      th: "ถนนสายหลักยาว 5 กิโลเมตร มี 8 ทางแยก ศูนย์ควบคุมกลางกำหนดรอบสัญญาณไฟและค่า Offset ให้สอดคล้องกันทั้งแนว และปรับตามสภาพจราจรเช้า–เย็นที่ต่างกัน",
